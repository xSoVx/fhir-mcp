...
5. **Audit logging** — every allowed *and* denied operation is recorded, on stderr by default. Ship and retain that stream; the server does not. The `resourceIdHash` field is a keyed HMAC-SHA256; an earlier revision used an unsalted truncated SHA-256 that was reversible. Audit records still describe PHI access, so retain them accordingly.
...
8. **Free text is masked** — `note[]`, `code.text`, `dosageInstruction[].text`, `report[].display`, `description` and `presentedForm[].title`/`.url` were previously unmasked on Condition, MedicationRequest, Procedure, CarePlan and DiagnosticReport. Free-text elements are now covered globally rather than per resource type, so a type added later inherits the protection.
