...
> 2. **Configure a caller principal.** Set `MCP_SERVICE_PRINCIPAL_ID` and `MCP_SERVICE_PRINCIPAL_SCOPES` alongside `PHI_MODE`. Without them the server starts, answers `fhir.capabilities` and terminology calls, and denies **every** IDENTIFIABLE read with `HEALTHCARE_COMPLIANCE_VIOLATION`. Several examples in this guide predate that requirement and set only `PHI_MODE`; add the principal variables to each.
...
4. **HTTP authentication is a shared secret** — `AUTH_TOKEN` is one static bearer token with no expiry, no rotation and no per-user attribution. **If it is unset the bridge accepts unauthenticated requests.** There is no OAuth2, SMART on FHIR, PKCE or client-credentials support in this repository, despite what earlier documentation claimed; `identity.ts` defines the seam a real token issuer would plug into later.
